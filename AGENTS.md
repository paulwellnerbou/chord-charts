...
- The song search box (`songSearch*` in `index.html`) calls an external API for song → chord-progression lookups; that backend is a separate, unpublished repo. Don't add backend logic here — the frontend only calls its HTTP API and must keep working with that API absent.
