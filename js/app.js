...
function generate(){
  ...
  tokens.forEach(tok=>{
    ...
    const voicings = findVoicings(parsed.requiredPCs, parsed.bassPC, tuning, maxFretValue, maxSpanValue, MAX_VOICINGS, allowMutedValue);
...
// The chooser's copies and the Options panel's are two faces of one setting,
// so every change writes back to both.
function syncVoicingSearchControls(){
  syncFretSpanControls('voicingSearch', maxFretValue, maxSpanValue);
  syncFretSpanControls('voicingModal', maxFretValue, maxSpanValue);
  document.getElementById('voicingSearchMuted').checked = allowMutedValue;
  document.getElementById('voicingModalMuted').checked = allowMutedValue;
}
...
function renderVoicingTiles(card, result){
  ...
  const voicings = findVoicings(result.requiredPCs, result.bassPC, tuning, maxFretValue, maxSpanValue, MAX_VOICINGS, allowMutedValue);
...
['voicingModalMuted', 'voicingSearchMuted'].forEach(id=>{
  document.getElementById(id).addEventListener('change', e=>{
    allowMutedValue = e.target.checked;
    syncVoicingSearchControls();
    generate();
  });
});
...
initStepper('voicingSearchMaxFret', stepMaxFret);
initStepper('voicingSearchMaxSpan', stepMaxSpan);
...
setupInfoPopover('voicingSearchMaxFretInfoWrap', 'voicingSearchMaxFretInfoBtn');
setupInfoPopover('voicingSearchMaxSpanInfoWrap', 'voicingSearchMaxSpanInfoBtn');
setupInfoPopover('voicingSearchMutedInfoWrap', 'voicingSearchMutedInfoBtn');
...
if(typeof savedSettings.allowMuted === 'boolean'){
  allowMutedValue = savedSettings.allowMuted;
}
...
const TIPS = [
  ...
  { emoji:'🔍', chordsOnly:true, text:'Raise “Search up to fret” in Options — or in the voicing chooser behind a card\'s arrows — to unlock alternate voicings further up the neck.' },
  { emoji:'📏', chordsOnly:true, text:'Fingers too short? Lower “Max stretch” in Options to skip voicings that need a wide stretch.' },
  { emoji:'🔇', chordsOnly:true, text:'Power chords hard to finger? Switch on “Allow muted strings” in Options and two- and three-note chords can skip strings instead of doubling notes.' },
